...
def get_last_time() -> int:
    try:
        seconds_cached = open(os.path.join(CACHE_FOLDER, "last_recheck.int"), 'rb')
    except:
        return 0
...
def set_last_time(new_unix_time: int):
    seconds_cached = open(os.path.join(CACHE_FOLDER, "last_recheck.int"), 'wb')
...
def get_last_charge() -> str:
    try:
        charge_cached = open(os.path.join(CACHE_FOLDER, "last_charge.txt"), 'r')
    except:
        return ''
...
def set_last_charge(charge: str):
    charge_cached = open(os.path.join(CACHE_FOLDER, "last_charge.txt"), 'w')
...
def main():
    ...
    if (not last_unix_time or not charge) or (len(sys.argv) > 2 and sys.argv[2] == "force-refresh") or (unix_time - last_unix_time) > UPDATE_PERIOD_SECONDS:
